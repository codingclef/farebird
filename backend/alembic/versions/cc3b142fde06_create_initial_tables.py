...
Revision ID: cc3b142fde06
Revises: 
Create Date: 2026-03-27 16:21:00.290253
...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_watched_routes_id'), 'watched_routes', ['id'], unique=False)
    ...
    sa.Column('return_date', sa.String(length=10), nullable=False),
    sa.Column('checked_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.ForeignKeyConstraint(['route_id'], ['watched_routes.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_watched_routes_id'), table_name='watched_routes')
